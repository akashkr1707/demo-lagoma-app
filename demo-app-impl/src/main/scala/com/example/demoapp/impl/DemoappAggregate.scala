...
/**
  * This provides an event sourced behavior. It has a state, [[DemoappState]], which
  ...
  */
object DemoappBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[DemoappCommand]): Behavior[DemoappCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, DemoappEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[DemoappCommand, DemoappEvent, DemoappState](
        persistenceId = persistenceId,
        emptyState = DemoappState.initial,
...
/**
  ...
  */
case class DemoappState(message: String, timestamp: String) {
  def applyCommand(cmd: DemoappCommand): ReplyEffect[DemoappEvent, DemoappState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: DemoappEvent): DemoappState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[DemoappEvent, DemoappState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[DemoappEvent, DemoappState] =
    Effect
...
object DemoappState {
  ...
  /**
    ...
    */
  def initial: DemoappState = DemoappState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[DemoappCommand]("DemoappAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[DemoappState] = Json.format
}

/**
  * This interface defines all the events that the DemoappAggregate supports.
  */
sealed trait DemoappEvent extends AggregateEvent[DemoappEvent] {
  def aggregateTag: AggregateEventTag[DemoappEvent] = DemoappEvent.Tag
}

object DemoappEvent {
  val Tag: AggregateEventTag[DemoappEvent] = AggregateEventTag[DemoappEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends DemoappEvent
...
/**
  ...
  */
trait DemoappCommandSerializable

/**
  * This interface defines all the commands that the DemoappAggregate supports.
  */
sealed trait DemoappCommand
    extends DemoappCommandSerializable
...
/**
  ...
  * application loader.
  */
object DemoappSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[DemoappState],
